...
class PointNetPlusPlus(nn.Module):
    def __init__(self, feature_dim=128, dim=3, hidden_dim=None, scatter_type=None, unet_type='alto',
            unet_kwargs=None, plane_resolution=None):
        super(PointNetPlusPlus, self).__init__()

        self.sa1 = PointNetSetAbstraction(npoint=512, radius=0.2, nsample=32, in_channel=dim + 3, mlp=[64, 64, 128], group_all=False)
        self.sa2 = PointNetSetAbstraction(npoint=128, radius=0.4, nsample=64, in_channel=128 + 3, mlp=[128, 128, 256], group_all=False)
        self.sa3 = PointNetSetAbstraction(npoint=None, radius=None, nsample=None, in_channel=256 + 3, mlp=[256, 512, 1024], group_all=True)
        self.fp3 = PointNetFeaturePropagation(in_channel=1280, mlp=[256, 256])
        self.fp2 = PointNetFeaturePropagation(in_channel=384, mlp=[256, 128])
        self.fp1 = PointNetFeaturePropagation(in_channel=128, mlp=[128, 128, feature_dim])
    ...
    def generate_plane_features(self, index_dict, c, plane):
        ...
        return fea_plane.reshape(c.size(0), self.feature_dim, self.reso_plane, self.reso_plane)

    def forward(self, xyz):
        coord, index = {}, {}

        coord['xy'] = xyz.clone()[:, :, [0, 1]]
        ...
        l0_points = l0_points.permute(0, 2, 1)  # [B, N, C]

        fea = {}
        fea_plane = self.generate_plane_features(index, l0_points, plane='xy')

        if self.unet_type == 'unet':
            fea['xy'] = self.unet(fea_plane)
        elif self.unet_type == 'alto':
            fea['xy'] = self.unet(xyz.permute(0, 2, 1), {'xy': fea_plane}, l0_points)
